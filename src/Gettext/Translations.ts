...
import TranslationsStats from './TranslationsStats';
...
export default class Translations {
     ...
     */
    public static defaultHeaders: SortedDictionary;
     ...
     * The gettext headers.
     */
    private readonly headers: SortedDictionary;

    /**
     * The list of translations
     */
    private readonly list: SortedDictionary;
     ...
     */
    private constructor(headers?: SortedDictionary, list?: SortedDictionary) {
        this.headerComments = [];
        this.headers = headers ? headers : new SortedDictionary();
        this.list = list ? list : new SortedDictionary();
        this.charset = '';
    ...
    public setLanguage(languageId: string, overwriteExistingPluralsDefinition?: boolean): boolean {
        let result: boolean;
        let plural: Plural | null = null;
        if (languageId) {
            let localeId = LocaleId.parse(languageId);
            if (localeId !== null) {
                plural = Plural.search(localeId);
     ...
     */
    public add(translation: Translation) {
    ...
    public toPot(): Translations {
        ...
        this.getTranslations().forEach((translation: Translation): void => {
     ...
     */
    public getStats(): TranslationsStats {
        return new TranslationsStats(this);
    }
}
Translations.defaultHeaders = new SortedDictionary();
