import { Gettext as GettextL } from './Language';
import { Gettext as GettextS } from './Script';
import { Gettext as GettextT } from './Territory';
...
export namespace Gettext {
    ...
    export class LocaleId {
        ...
        public getName(): string {
            ...
            let name = GettextL.Language.getName(this.languageId + '-' + this.scriptId + '-' + this.territoryId);
            if (name.length === 0) {
                name = GettextL.Language.getName(this.languageId + '-' + this.territoryId);
                if (name.length > 0) {
                    addScript = true;
                } else {
                    name = GettextL.Language.getName(this.languageId + '-' + this.scriptId);
                    if (name.length > 0) {
                        addTerritory = true;
                    } else {
                        name = GettextL.Language.getName(this.languageId);
                        if (name.length > 0) {
                            addScript = true;
                            addTerritory = true;
                        } else {
            ...
            if (addTerritory && this.territoryId.length > 0) {
                name += ' (' + (GettextT.Territory.getName(this.territoryId) || this.territoryId) + ')';
            }
            if (addScript && this.scriptId.length > 0) {
                name += ' [' + (GettextS.Script.getName(this.scriptId) || this.scriptId) + ']';
            }
            return name;
        }
    }
